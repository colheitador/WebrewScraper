...
def func_gerar_csv(filtro:str): ## o parâmetro filtro deverá ser uma f string (exemplo:    gerar_csv(f"WHERE nome_do_produto_c LIKE ('%{nome}%')")) 

    def fazer_linha_1_csv(filtro:str):
        ...
        for cada_linha_tabela_de_fermentos in informacoes_sobre_fermentos[0:1]:
            ...
            # agora adicionar os dicionários (com conteúdo de fermentos) a uma lista para mostrar na tela
            #resultados.append(cada_dicionario_de_fermentos)

            tabela_detalhes = pd.DataFrame(cada_dicionario_de_fermentos) ## para criar uma tabela a partir desse dicionário

            
        tabela_em_csv = tabela_detalhes.to_csv("ultima_busca.csv", sep = ',',index=False) ## para gerar um .csv a partir dessa tabela

    def fazer_linhas_restantes_csv(filtro:str):
        ## parte 2: copias as linhas restantes da tabela de fermentos pro .csv

        tabela_detalhes = pd.read_csv("ultima_busca.csv") ## 'index_col=0' é pra que o pandas não adicione mais uma coluna de índice
    ...
    fazer_linha_1_csv(filtro) # executa a parte 1
    fazer_linhas_restantes_csv(filtro) # executa a parte 2
#-----------------------------------------------------------------------------------------------------------------------------------------------------#
...
def func_pesquisar_por_nome(nome: str):

    #nome = input("Digite o nome do fermento ou uma sequência de caracteres presentes no nome do fermento desejado:") # esta linha está comentada porque ela não é necessária para a API, mas ainda pode ser útil durante a realização de testes em jupyter notebook

    # para isso, vamos criar um filtro para que o usuário busque por uma palavra ou sequencia de caracteres presentes no do fermento	
    ...
    for cada_linha_tabela_de_fermentos in informacoes_sobre_fermentos:
        ...
        # agora adicionar os dicionários (com conteúdo de fermentos) a uma lista para mostrar na tela
        resultados.append(cada_dicionario_de_fermentos)
        #print(cada_linha_tabela_de_fermentos)
        #resultados.append(cada_linha_tabela_de_fermentos)
    
    if (len(resultados) == 0): func_csv_sem_resultados() ## se a busca não retornar nenhum resultado, gera-se uma tabela vazia
    else: func_gerar_csv(f"WHERE nome_do_produto_c LIKE ('%{nome}%')")
    ...
    # as linhas abaixo estão comentadas porque elas não são necessárias na API, mas podem ser úteis para testes em jupyter notebook
    # print(numero_de_resultados) # comentario do trabalho de startups: ##caso queira mostrar os resultados da busca em .json, comente esta linha e descomente a "return json.dumps(resultados)"
...
def func_pesquisar_por_preco_maximo(preco_maximo: str): ## não é preciso converter o preço para float aqui, pois o valor deve estar como string pra funcionar na busca por f string abaixo

    #preco_maximo = (input("Digite o preço máximo a ser exibido [separando os reais dos centavos por ponto (.) ]:")) # esta linha está comentada porque ela não é necessária para a API, mas ainda pode ser útil durante a realização de testes em jupyter notebook
    ...
    if (len(resultados) == 0): func_csv_sem_resultados() ## se a busca não retornar nenhum resultado, gera-se uma tabela vazia
    else: func_gerar_csv(f"WHERE preco_BRL <= {preco_maximo}")
...
def func_pesquisar_por_nome_e_preco(nome: str, preco_maximo: str):
    ...
    if (len(resultados) == 0): func_csv_sem_resultados() ## se a busca não retornar nenhum resultado, gera-se uma tabela vazia
    else: func_gerar_csv(f"WHERE nome_do_produto_c LIKE ('%{nome}%') AND preco_BRL <= {preco_maximo}")
...
def func_pesquisar_por_descricao(descricao: str):

    #nome = input("Digite uma palavra ou uma sequência de caracteres que devem estar presentes na descrição do fermento desejado:") # esta linha está comentada porque ela não é necessária para a API, mas ainda pode ser útil durante a realização de testes em jupyter notebook
    ...
    if (len(resultados) == 0): func_csv_sem_resultados() ## se a busca não retornar nenhum resultado, gera-se uma tabela vazia
    else: func_gerar_csv(f"WHERE descricao_c LIKE ('%{descricao}%')")
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------#
def func_deletar_mariacervejeira():
...
#-----------------------------------------------------------------------------------------------------------------------------------------------------#
def func_deletar_piquiribrewshop():
